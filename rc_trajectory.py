...
class WrapperTerminal:
    ...
    def prompt_api_key(self, prefilled_key: str | None = None) -> ChatGPTClient:
        """Request an API key before any wrapper interaction occurs.

        ``prefilled_key`` allows non-interactive or PowerShell callers to supply
        the key via a flag or environment variable, avoiding prompt/echo
        differences across shells.
        """

        api_key = (prefilled_key or self.input_fn("Enter OpenAI API key (kept local): ")).strip()
        if not api_key:
...
def launch_wrapper_terminal(
    *,
    ...
    resolved_input_fn = input_fn or (
        lambda prompt: getpass(prompt) if "API key" in prompt else input(prompt)
    )
    resolved_output_fn = output_fn or print

    terminal = WrapperTerminal(
        default_wrapper,
        client_builder=client_builder,
        input_fn=resolved_input_fn,
        output_fn=resolved_output_fn,
        max_tokens=max_tokens,
    )

    try:
        client = terminal.prompt_api_key(prefilled_key=api_key)
    except ValueError as exc:  # pragma: no cover - user-facing
        resolved_output_fn(str(exc))
        sys.exit(1)
...
def _build_arg_parser() -> ArgumentParser:
    parser = ArgumentParser(description="RC trajectory demo and wrapper terminal")
    parser.add_argument(
        "--wrapper",
        action="store_true",
        help=(
            "Launch the interactive wrapper terminal (PowerShell-friendly) "
            "instead of the RC trajectory demo."
        ),
    )
    parser.add_argument(
        "--api-key",
        help="OpenAI API key; if omitted you will be prompted (works in PowerShell).",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
        default=8,
        help="Max tokens for wrapper responses (default: 8)",
    )
    return parser
...
def cli(argv: List[str] | None = None) -> None:
    ...
    if args.wrapper:
        launch_wrapper_terminal(api_key=args.api_key, max_tokens=args.max_tokens)
    else:
        main()
