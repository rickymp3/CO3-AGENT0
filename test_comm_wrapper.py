...
def test_prompt_api_key_accepts_prefilled_key():
    wrapper = make_wrapper()
    builder = MagicMock()
    client = MagicMock()
    builder.return_value = client

    def fail_input(prompt: str) -> str:  # pragma: no cover - defensive
    ...
    built = terminal.prompt_api_key(prefilled_key="abc123")

    assert built is client
    builder.assert_called_once_with(api_key="abc123")


@patch("rc_trajectory.launch_wrapper_terminal")
def test_cli_invokes_wrapper_when_flag_set(mock_launch):
    cli(["--wrapper", "--api-key", "ps-key", "--max-tokens", "5"])

    mock_launch.assert_called_once_with(api_key="ps-key", max_tokens=5)
